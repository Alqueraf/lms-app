...
class ChooseRecipientsFragment : BaseSyncFragment<Recipient, ChooseRecipientsPresenter, ChooseRecipientsView, RecipientViewHolder, ChooseMessageRecipientRecyclerAdapter>(), ChooseRecipientsView {
            ...
            when (recipient.recipientType) {
                Recipient.Type.group -> {
                    ...
                    if (recipient.userCount > 0) {
                        if (isCheckbox) {
                            presenter.addOrRemoveRecipient(recipient, position)
                        } else {
    ...
    private fun setupToolbar(view: View) {
        with(view.toolbar) {
            ...
            // Apply toolbar theme
            ViewStyler.themeToolbarBottomSheet(activity, resources.getBoolean(R.bool.isDeviceTablet), this, Color.BLACK, false)
        }
    }
    ...
    override fun onReadySetGo(presenter: ChooseRecipientsPresenter) {
        ...
        if (arguments.getParcelableArrayList<Parcelable>(RECIPIENT_LIST) != null) {
            val recipients = arguments.getParcelableArrayList<Recipient>(RECIPIENT_LIST)
    ...
    override fun getPresenterFactory(): PresenterFactory<ChooseRecipientsPresenter> =
            ChooseRecipientsPresenterFactory(arguments.getString(CONTEXT_ID))

    override fun onPresenterPrepared(presenter: ChooseRecipientsPresenter) {
        mRecyclerView = RecyclerViewUtils.buildRecyclerView(activity.window.decorView.rootView,
                context, adapter, presenter, R.id.swipeRefreshLayout, R.id.recyclerView, R.id.emptyPandaView, getString(R.string.no_items_to_display_short))
    }

    override fun getAdapter(): ChooseMessageRecipientRecyclerAdapter {
        if (mRecyclerAdapter == null) {
            mRecyclerAdapter = ChooseMessageRecipientRecyclerAdapter(context, presenter, mAdapterToFragmentCallback)
        }
