...
class MessageThreadFragment : BaseSyncFragment<Message, MessageThreadPresenter, MessageThreadView, MessageHolder, MessageAdapter>(), MessageThreadView, Identity {
        ...
        override fun onAttachmentClicked(action: AttachmentView.AttachmentAction, attachment: Attachment) {
            if (action == AttachmentView.AttachmentAction.PREVIEW) {
                ...
                if (PermissionUtils.hasPermissions(activity, PermissionUtils.WRITE_EXTERNAL_STORAGE)) {
                    // Download media
                    MediaDownloader.download(context, attachment.url, attachment.filename, attachment.filename)
                } else {
    ...
    override fun getPresenterFactory(): PresenterFactory<MessageThreadPresenter> {
        if (arguments?.containsKey(Const.CONVERSATION_ID) == true) {
            ...
            return MessageThreadPresenterFactory(arguments.getLong(Const.CONVERSATION_ID, 0)) // No way to know from the push notification where in the conversation we should be; position = 0
        }

        return MessageThreadPresenterFactory(arguments.getParcelable<Parcelable>(Const.CONVERSATION) as Conversation, arguments.getInt(Const.POSITION))
    }

    override fun onPresenterPrepared(presenter: MessageThreadPresenter) {
        conversationScope = arguments.getString(Const.SCOPE)
    ...
    private fun setupRecyclerView() {
        recyclerView = RecyclerViewUtils.buildRecyclerView(activity.window.decorView.rootView, context, adapter,
        ...
        recyclerView?.let {
            ...
            val dividerItemDecoration = DividerItemDecoration(it.context, (it.layoutManager as LinearLayoutManager).orientation)
    ...
    override fun getAdapter(): MessageAdapter {
        if (mAdapter == null) {
            mAdapter = MessageAdapter(context, presenter, conversation!!, adapterCallback)
        }
        return mAdapter
    }
