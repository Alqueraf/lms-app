...
class PeopleListFragment : BaseSyncFragment<User, PeopleListPresenter, PeopleListView, UserViewHolder, PeopleListRecyclerAdapter>(), PeopleListView, SearchView.OnQueryTextListener {
    ...
    override fun onHandleBackPressed(): Boolean {
        ...
        if (peopleListToolbar.menu.findItem(R.id.search).isActionViewExpanded) {
            peopleListToolbar.menu.findItem(R.id.search).collapseActionView()
            return true
        }
    ...
    private fun setupViews() {
        val canvasContext = arguments.getParcelable<CanvasContext>(Const.CANVAS_CONTEXT)
        ...
        peopleListToolbar.menu.findItem(R.id.peopleFilterMenuItem)?.setOnMenuItemClickListener {
            ...
            PeopleListFilterDialog.getInstance(activity.supportFragmentManager, presenter.canvasContextListIds, canvasContext, true) { canvasContexts ->
                mCanvasContextsSelected = ArrayList()
                mCanvasContextsSelected!!.addAll(canvasContexts)

                presenter.canvasContextList = mCanvasContextsSelected
        ...
        ViewStyler.themeToolbar(activity, peopleListToolbar, ColorKeeper.getOrGenerateColor(canvasContext), Color.WHITE)
    ...
    override fun getPresenterFactory(): PresenterFactory<PeopleListPresenter> =
            PeopleListPresenterFactory(arguments.getParcelable<Parcelable>(Const.CANVAS_CONTEXT) as CanvasContext)
    ...
    override fun getAdapter(): PeopleListRecyclerAdapter {
        if (mAdapter == null) {
            mAdapter = PeopleListRecyclerAdapter(context, presenter, AdapterToFragmentCallback { user, _ ->
                val canvasContext = arguments.getParcelable<CanvasContext>(Const.CANVAS_CONTEXT)
                ...
                RouteMatcher.route(context, Route(null, StudentContextFragment::class.java, canvasContext, bundle))
            })
        }
        return mAdapter
    }
