...
class EditFileFolderFragment : BasePresenterFragment<
    ...
    lateinit private var updateFileFolder: FileFolder

    private val dateClickListener: (View, Boolean) -> Unit = { view, isLockDate ->
        DatePickerDialogFragment.getInstance(activity.supportFragmentManager, if (isLockDate) lockDate else unlockDate) { year, month, dayOfMonth ->
    ...
    private val timeClickListener: (View, Boolean) -> Unit = { view, isLockDate ->
        TimePickerDialogFragment.getInstance(activity.supportFragmentManager, if (isLockDate) lockDate else unlockDate) { hour, min ->
    ...
    private fun setupViews() {
        ...
        deleteWrapper.setOnClickListener {
            ConfirmDeleteFileFolderDialog.show(activity.supportFragmentManager, presenter.isFile) {
        ...
        // Apply theming
        ViewStyler.themeEditText(activity, titleEditText, ThemePrefs.brandColor)
        ViewStyler.themeEditText(activity, copyrightEditText, ThemePrefs.brandColor)

        ViewStyler.themeSpinner(activity, accessSpinner, ThemePrefs.brandColor)
        ViewStyler.themeSpinner(activity, restrictedAccessSpinner, ThemePrefs.brandColor)
        ViewStyler.themeSpinner(activity, usageRightsSpinner, ThemePrefs.brandColor)
        ViewStyler.themeSpinner(activity, licenseSpinner, ThemePrefs.brandColor)
    ...
    private fun setupAccess() {
        val spinnerAdapter = ArrayAdapter.createFromResource(activity, R.array.fileAccessTypes, R.layout.simple_spinner_item)
        spinnerAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
        accessSpinner.adapter = spinnerAdapter
        ViewStyler.themeSpinner(context, accessSpinner, ThemePrefs.brandColor)
        ...
        val initialPosition =
                when {
                    ...
                    presenter.currentFileOrFolder.isHidden ||
                            presenter.currentFileOrFolder.unlockAt != null ||
                            presenter.currentFileOrFolder.lockAt != null -> {
                        lockDate = presenter.currentFileOrFolder.lockAt
                        unlockDate = presenter.currentFileOrFolder.unlockAt
    ...
    private fun setupRestrictedAccess() {
        val spinnerAdapter = LongNameArrayAdapter.createFromResource(activity, if (presenter.isFile) R.array.fileRestrictedTypes else R.array.folderRestrictedTypes, R.layout.simple_spinner_item)
        ...
        val initialPosition =
                when {
                    ...
                    presenter.currentFileOrFolder.lockAt != null || presenter.currentFileOrFolder.unlockAt != null -> {
                        if (presenter.currentFileOrFolder.lockAt != null) {
                            lockDateEditText.setText(mDateFormat.format(presenter.currentFileOrFolder.lockAt))
                            lockTimeEditText.setText(mTimeFormat.format(presenter.currentFileOrFolder.lockAt))
                        }
                        if (presenter.currentFileOrFolder.unlockAt != null) {
                            unlockDateEditText.setText(mDateFormat.format(presenter.currentFileOrFolder.unlockAt))
                            unlockTimeEditText.setText(mTimeFormat.format(presenter.currentFileOrFolder.unlockAt))
                        }
    ...
    private fun setupUsageRights() {
        val spinnerAdapter = ArrayAdapter.createFromResource(activity, R.array.fileUsageRightsTypes, R.layout.simple_spinner_item)
        spinnerAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
        usageRightsSpinner.adapter = spinnerAdapter
        ViewStyler.themeSpinner(context, usageRightsSpinner, ThemePrefs.brandColor)
    ...
    private fun setupLicenses(licenses: List<License>) {
        ...
        val spinnerAdapter = ArrayAdapter(activity, R.layout.simple_spinner_item, presenter.licenseList.filter { it.name.contains("CC") }.map { it.name })
        spinnerAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
        licenseSpinner.adapter = spinnerAdapter
        ViewStyler.themeSpinner(context, licenseSpinner, ThemePrefs.brandColor)
    ...
    private fun saveFileFolder() {
        ...
        updateFileFolder.name = titleEditText.text.toString()

        mAccessStatus.lockAt = APIHelper.dateToString(lockDate) ?: ""
        mAccessStatus.unlockAt = APIHelper.dateToString(unlockDate) ?: ""
