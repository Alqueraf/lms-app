...
class CreateOrEditAnnouncementFragment :
    ...
    private fun setupDescription() {
        if (CanvasWebView.containsLTI(presenter.announcement.message.orEmpty(), "UTF-8")) {
            announcementRCEView.setHtml(
                    DiscussionUtils.createLTIPlaceHolders(context, presenter.announcement.message.orEmpty(), { _, placeholder ->
    ...
    private fun setupAllowCommentsSwitch() {
        ...
        allowCommentsSwitch.isChecked = !presenter.announcement.isLocked
        allowCommentsSwitch.setOnCheckedChangeListener { _, isChecked ->
            presenter.announcement.isLocked = !isChecked
    ...
    private fun setupUsersMustPostSwitch() {
        with (usersMustPostSwitch) {
            applyTheme()
            isChecked = presenter.announcement.isRequireInitialPost
            enableUsersMustPostSwitch(!presenter.announcement.isLocked)
    ...
    override fun onSectionsLoaded() {
        ...
        sections?.onClick {
            SectionPickerDialog.show(fragmentManager, presenter.courseSections, presenter.getSelectedSections()) {
