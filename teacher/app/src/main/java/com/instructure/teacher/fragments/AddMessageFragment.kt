...
class AddMessageFragment : BasePresenterFragment<AddMessagePresenter, AddMessageView>(), AddMessageView {
    ...
    private var isMessageStudentsWho = false
    private var isPersonalMessage = false
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        isNewMessage = arguments.getBoolean(Const.COMPOSE_FRAGMENT)
        isPersonalMessage = arguments.getBoolean(MESSAGE_STUDENTS_WHO_CONTEXT_IS_PERSONAL)
        isMessageStudentsWho = arguments.getBoolean(MESSAGE_STUDENTS_WHO)
        ...
        if (savedInstanceState != null && !isNewMessage) {
            ...
            currentMessage = arguments.getParcelable(Const.MESSAGE_TO_USER)
    ...
    override fun getPresenterFactory(): PresenterFactory<AddMessagePresenter> {
        ...
        val isReply = arguments.getBoolean(KEY_IS_REPLY, false)
    ...
    override fun onReadySetGo(presenter: AddMessagePresenter) {
        ...
        if (!isNewMessage && !isMessageStudentsWho) {
            ...
            if (isPersonalMessage) {
                subjectView.setGone()
                editSubject.setVisible()
                editSubject.setText(arguments.getString(MESSAGE_STUDENTS_WHO_SUBJECT))
            } else {
                subjectView.text = arguments.getString(MESSAGE_STUDENTS_WHO_SUBJECT)
            }
        }
        ...
        if (chipsAdapter == null) {
            chipsAdapter = RecipientAdapter(context)
        }
        ...
        contactsImageButton.setOnClickListener {
            ...
            RouteMatcher.route(context, Route(ChooseRecipientsFragment::class.java, canvasContext, ChooseRecipientsFragment.createBundle(canvasContext!!, recipientsFromRecipientEntries)))
        }
    ...
    private fun setupToolbar() {
        ...
        ViewStyler.themeToolbarBottomSheet(activity, resources.getBoolean(R.bool.isDeviceTablet), toolbar, Color.BLACK, false)
    ...
    fun handleExit() {
        ...
        if (selectedCourse != null || editSubject.text.isNotEmpty() || message.text.isNotEmpty() || presenter.attachments.isNotEmpty()) {
            ...
            UnsavedChangesExitDialog.show(activity.supportFragmentManager) {
    ...
    internal fun sendMessage() {
        ...
        if (isNewMessage || isMessageStudentsWho) {
            ...
            if (recipient.selectedRecipients.size > 1 && sendIndividually) {
                isBulk = true
            } else {
                for (entry in recipient.selectedRecipients) {
                    if (entry.userCount > 1 && sendIndividually) {
                        isBulk = true
                        break
                    }
                }
            ...
            if (isMessageStudentsWho) {
                ...
                contextId = arguments.getString(MESSAGE_STUDENTS_WHO_CONTEXT_ID, "")
    ...
    private fun addRecipients(newRecipients: ArrayList<Recipient>) {
        ...
        addRecipient@ for (recipientUser in newRecipients) {

            // Skip if this recipient is already added
            @Suppress("LoopToCallChain") // Suppress converting to use std methods; Compiler thinks the code is jumping outside the method
            for (entry in selectedRecipients) {
                if (entry.destination == recipientUser.stringId) {
                    continue@addRecipient
                }
            }

            // Create RecipientEntry from Recipient and add to list
            val recipientEntry = RecipientEntry(recipientUser.idAsLong, recipientUser.name, recipientUser.stringId, "", recipientUser.avatarURL, 0, 0, true,
                    if (recipientUser.commonCourses != null) recipientUser.commonCourses.keys else null,
                    if (recipientUser.commonGroups != null) recipientUser.commonGroups.keys else null)
            recipient.appendRecipientEntry(recipientEntry)
        }
    }
    ...
    fun onRecipientsUpdated(event: ChooseMessageEvent) {
        event.once(javaClass.simpleName) { recipients ->
            ...
            recipient.post { addRecipients(recipients) }
        }
    }
    ...
    companion object {
        ...
        fun newInstance(bundle: Bundle): AddMessageFragment =
