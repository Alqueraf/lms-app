...
class UploadFilesDialog : AppCompatDialogFragment() {
    ...
    private fun setupAllowedExtensions(view: View) {
        ...
        if (uploadType != FileUploadType.SUBMISSION_COMMENT && assignment != null && assignment?.allowedExtensions != null && (assignment?.allowedExtensions?.size ?: 0) > 0) {
            assignment!!.let {
    ...
    private fun setDialogMargins(window: Window) {
        ...
        val width = Math.round(screenWidth - convertDipsToPixels(2f, activity) * 2) // * 2 for margin on each side
    ...
    private fun onFromCameraClick() {
        if (!PermissionUtils.hasPermissions(activity, PermissionUtils.CAMERA)) {
        ...
        cameraImageUri = FileProvider.getUriForFile(context, context.packageName + Const.FILE_PROVIDER_AUTHORITY, file)
        ...
        if (isIntentAvailable(activity, intent.action)) {
    ...
    private fun onFromGalleryClick() {
        ...
        intent.setDataAndType(FileProvider.getUriForFile(context,
                context.packageName + Const.FILE_PROVIDER_AUTHORITY, file), "image/*")
    ...
    private fun checkIfFileSubmissionAllowed(): Boolean {
        return if (assignment != null) {
            assignment!!.submissionTypes.contains(Assignment.SUBMISSION_TYPE.ONLINE_UPLOAD)
        } else false
    }
    ...
    inner class FileRecyclerViewAdapter(private val onRemovedFileCallback: (FileSubmitObject, Int) -> Unit) : RecyclerView.Adapter<FileViewHolder>() {
        ...
        override fun onCreateViewHolder(parent: ViewGroup?, viewType: Int): FileViewHolder {
            return FileViewHolder(LayoutInflater.from(parent!!.context).inflate(R.layout.adapter_file_uploads, parent, false))
        }
