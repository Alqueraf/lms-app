...
class ExoAgent private constructor(val uri: Uri) {
     ...
     */
    fun attach(playerView: SimpleExoPlayerView, listener: ExoInfoListener) {
     ...
     */
    fun prepare(playerView: SimpleExoPlayerView) {
        if (mPlayer == null) preparePlayer()
        mPlayer?.switchSurface(playerView)
    }

    private fun SimpleExoPlayer.switchSurface(playerView: SimpleExoPlayerView) {
    ...
    private fun preparePlayer() {
        ...
        val trackSelectionFactory = AdaptiveTrackSelection.Factory(BANDWIDTH_METER)
        ...
        mPlayer?.addListener(object : Player.EventListener {
            ...
            override fun onTracksChanged(trackGroups: TrackGroupArray, trackSelections: TrackSelectionArray) {
                mIsAudioOnly = trackSelections.all.filter { it != null }.none { it.selectedFormat.sampleMimeType.startsWith("video") }
                if (mIsAudioOnly) mInfoListener?.setAudioOnly()
            }
            ...
            override fun onPlayerStateChanged(playWhenReady: Boolean, state: Int) {
                when (state) {
                    Player.STATE_BUFFERING -> currentState = ExoAgentState.BUFFERING
                    Player.STATE_ENDED -> {
                        mPlayer?.seekToDefaultPosition()
                        mPlayer?.stop()
                        mPlayer?.release()
                        currentState = ExoAgentState.ENDED
                    }
                    else -> currentState = ExoAgentState.READY
                }
    ...
    companion object {
        ...
        private val CONNECT_TIMEOUT = 8000

        private val READ_TIMEOUT = 8000
