...
object BitriseApps {
    ...
    fun getBuildLog(appSlug: String, build: BitriseBuildObject): ByteArray {
        ...
        return noAuthClient.newCall(request).execute().body().bytes()
    }
    ...
    fun getAppsForOrg(orgName: String = getEnv("BITRISE_ORG")): List<BitriseAppObject> {
        ...
        return appsList.filter { orgName.equals(it.owner.name, ignoreCase = true) }
    }

    fun getBuild(appSlug: String, buildSlug: String): BitriseSingleBuildData {
        return apiService.getBuild(appSlug, buildSlug).execute().body()
    }
    ...
    fun getBuilds(app: BitriseAppObject,
        ...
        while (next != null) {
            ...
            for (buildObject in response.body().data) {
            ...
            next = if (page == false || noMoreData) null else response.body().paging.next
        }
