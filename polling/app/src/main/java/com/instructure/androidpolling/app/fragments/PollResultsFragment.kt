...
class PollResultsFragment : PaginatedListFragment<PollChoice>() {
    ...
    private val pollList: ArrayList<Poll>? = null
    private val pollChoices: ArrayList<PollChoice>? = null
    private var sessionResults: Map<Long, Int>? = null
    private var totalSubmissions = 0
    private val hasSubmissions = false
    ...
    private val callback: ParentFragment.OnUpdatePollListener? = null
    ...
    private fun updateViews() {
        ...
        createdTime = pollSession!!.created_at.time
        if (pollSession!!.is_published) {
    ...
    private fun setupClickListeners() {
        publishPoll.setOnClickListener {
            if (pollSession!!.is_published) {
                ...
                startActivityForResult(PublishPollActivity.createIntent(activity, poll!!.id), Constants.PUBLISH_POLL_REQUEST)
            }
        }
        ...
        shareResults!!.setOnClickListener {
            ...
            if (!pollSession!!.has_public_results()) {
                PollsManager.updatePollSession(poll!!.id, pollSession!!.id, pollSession!!.course_id, pollSession!!.course_section_id, true, updatePollSessionCallback, true)
            } else {
                // Otherwise, turn them off
                PollsManager.updatePollSession(poll!!.id, pollSession!!.id, pollSession!!.course_id, pollSession!!.course_section_id, false, updatePollSessionCallback, true)
            }
        }
    }

    override fun getRootLayoutCode(): Int {
    ...
    override fun configureViews(rootView: View) {
        ...
        if (arguments != null && poll == null) {
            ...
            pollSession = arguments.getParcelable(Constants.POLL_SESSION)
        }
    ...
    override fun getRowViewForItem(item: PollChoice?, convertView: View?, position: Int): View {
        ...
        return PollResultsRowFactory.buildRowView(layoutInflater(), activity, item!!.text, (result * 100).toInt(), item.is_correct, convertView, position)
    }
