...
class StudentPollFragment : ParentFragment() {
    ...
    private fun setupViews() {
        answers = TreeSet()
        answerAdapter = AnswerAdapter(activity, answers!!)
    ...
    private fun updateViewsSubmitted() {
        ...
        pollQuestion!!.setBackgroundColor(resources.getColor(R.color.white))
    ...
    private inner class AnswerAdapter(context: Activity, private val views: TreeSet<AnswerValue>) : BaseAdapter() {
        ...
        override fun getView(position: Int, convertView: View, viewGroup: ViewGroup): View {
            ...
            if (getItemViewType(position) == 0) {
                ...
                return StudentPollResultsRowFactory.buildRowView(layoutInflater, activity, answerValue.value, (numAnswered * 100).toInt(), answerValue.isCorrect, selected, hasCorrectAnswer, convertView, position)
            } else {
                return StudentPollRowFactory.buildRowView(layoutInflater, activity, itemAtPosition(position) ?: AnswerValue(), position, hasSubmitted, pollSession!!.is_published, convertView)
            }
        }
    }
    ...
    private fun setupCallbacks() {
            ...
            override fun onResponse(response: retrofit2.Response<PollChoiceResponse>, linkHeaders: LinkHeaders, type: ApiType) {
                ...
                if (pollChoices != null) {
                    for (pollChoice in pollChoices) {
                        addAnswer(pollChoice.text, pollChoice.id, pollChoice.is_correct, pollChoice.position)
        ...
        pollSubmissionCallback = object : StatusCallback<PollSubmissionResponse>() {
            override fun onResponse(response: Response<PollSubmissionResponse>, linkHeaders: LinkHeaders, type: ApiType) {
                ...
                ApplicationManager.savePollSubmission(activity, pollSession!!.id, response.body()!!.pollSubmissions[0].poll_choice_id)
        ...
        pollSessionCallback = object : StatusCallback<PollSessionResponse>() {
            override fun onResponse(response: Response<PollSessionResponse>, linkHeaders: LinkHeaders, type: ApiType) {
                ...
                if (pollSessions != null) {
                    showResults = pollSessions[0].has_public_results()
