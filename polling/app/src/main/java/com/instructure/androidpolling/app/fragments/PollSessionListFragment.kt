...
class PollSessionListFragment : PaginatedListFragment<PollSession>() {
    ...
    private fun setupClickListeners() {
        publishPoll.setOnClickListener { startActivityForResult(PublishPollActivity.createIntent(activity, poll!!.id), Constants.PUBLISH_POLL_REQUEST) }
    }
    ...
    private fun generateCSV() {
        ...
        for (i in 0 until itemCount) {
            ...
            if (pollSession.poll_submissions != null) {
                for (pollSubmission in pollSession.poll_submissions) {
                    // Now add all the necessary stuff to the csv string
                    csv += poll!!.question + ","
                    csv += pollSession.id.toString() + ","
                    csv += courseMap?.get(pollSession.course_id)?.name + ","
                    csv += sectionMap?.get(pollSession.course_section_id)?.name + ","
                    csv += pollSubmission.user_id.toString() + ","
                    // Make sure we have the poll choice information so we can include the poll choice text instead
                    // of just an id
                    csv += if (pollChoiceMap.containsKey(pollSubmission.poll_choice_id)) {
                        pollChoiceMap[pollSubmission.poll_choice_id]?.text + ","
                    } else {
                        pollSubmission.poll_choice_id.toString() + ","
                    }
                    csv += pollSubmission.created_at.toString() + "\n"
                }
            } else {
                csv += poll!!.question + ","
                csv += pollSession.id.toString() + ","
                csv += courseMap?.get(pollSession.course_id)?.name + ","
                csv += sectionMap[pollSession.course_section_id]?.name + ","
                csv += "" + ","
                csv += "" + ","
                csv += pollSession.created_at.toString() + "\n"
            }
    ...
    override fun configureViews(rootView: View) {
        ...
        if (arguments != null) {
            poll = arguments.getParcelable(Constants.POLL_DATA)
            if (poll != null) {
    ...
    override fun getRowViewForItem(item: PollSession, convertView: View?, position: Int): View {
        ...
        val courseName = courseMap?.get(item.course_id)?.name ?: ""

        var sectionName = ""
        if (sectionMap.containsKey(item.course_section_id)) {
            sectionName = sectionMap[item.course_section_id]!!.name
        }

        return PollSessionRowFactory.buildRowView(layoutInflater(), activity, courseName, sectionName, item.is_published, convertView)
    }
            ...
            override fun onResponse(response: Response<PollSessionResponse>, linkHeaders: LinkHeaders, type: ApiType) {
                ...
                if (pollSessions != null) {
                    for (pollSession in pollSessions) {
                        addItem(pollSession)
                        SectionManager.getSection(pollSession.course_id, pollSession.course_section_id, sectionCallback, true)
                    }
                }
            }
