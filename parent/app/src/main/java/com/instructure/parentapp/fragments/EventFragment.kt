...
class EventFragment : ParentFragment(), TimePickerFragment.TimePickerFragmentListener, TimePickerFragment.TimePickerCancelListener, DatePickerFragment.DatePickerFragmentListener, DatePickerFragment.DatePickerCancelListener {
        ...
        checkedChangeListener = CompoundButton.OnCheckedChangeListener { _, isChecked ->
            if (isChecked) {
                ...
                if (scheduleItem.startAt != null) {
                    cal.time = scheduleItem.startAt
                }

                datePickerDialog = DatePickerFragment.newInstance(this@EventFragment, this@EventFragment)

                datePickerDialog!!.show(activity.fragmentManager, DATE_PICKER_EVENT_TAG)
    ...
    private fun cancelAlarm() {
        ...
        var subTitle = ""
        if (scheduleItem.startAt != null) {
            subTitle = DateHelper.getDateTimeString(context, scheduleItem.startAt)
        }
    ...
    private fun populateViews() {
        ...
        if (scheduleItem.isAllDay) {
            ...
            date2.text = getFullDateString(scheduleItem.endAt)
        } else {
            //Setup the calendar event start/end times
            if (scheduleItem.startAt != null && scheduleItem.endAt != null && scheduleItem.startAt!!.time != scheduleItem.endAt!!.time) {
                //Our date times are different so we display two strings
                date1.text = getFullDateString(scheduleItem.endAt)
                val startTime = DateHelper.getFormattedTime(context, scheduleItem.startAt)
                val endTime = DateHelper.getFormattedTime(context, scheduleItem.endAt)
                date2.text = "$startTime - $endTime"
            } else {
                date1.text = getFullDateString(scheduleItem.startAt)
        ...
        if (!TextUtils.isEmpty(content)) {
            ...
            if (resources.getBoolean(R.bool.isTablet)) {
                eventWebView.setBackgroundColor(ContextCompat.getColor(context,R.color.white))
            } else {
                eventWebView.setBackgroundColor(ContextCompat.getColor(context,R.color.canvasBackgroundLight))
            }
    ...
    override fun onDateSet(year: Int, month: Int, day: Int) {
        ...
        timePicker!!.show(activity.supportFragmentManager, TIME_PICKER_EVENT_TAG)
    }
    ...
    override fun onTimeSet(hourOfDay: Int, minute: Int) {
        ...
        try {
            ...
            val subTitle = if (scheduleItem.startAt != null) {
                DateHelper.getShortDateTimeStringUniversal(context, scheduleItem.startAt)
            } else {
                ""
            }
    ...
    companion object {
        ...
        private val DATE_PICKER_EVENT_TAG = "datePickerEvent"
        private val TIME_PICKER_EVENT_TAG = "timePickerEvent"
