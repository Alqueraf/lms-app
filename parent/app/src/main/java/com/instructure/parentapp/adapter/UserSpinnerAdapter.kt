...
class UserSpinnerAdapter(context: Context, users: Array<User>) : ArrayAdapter<User>(context, 0, users) {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        return (convertView ?: inflater.inflate(R.layout.spinner_user, parent, false)).apply {
    ...
    override fun getDropDownView(position: Int, convertView: View?, parent: ViewGroup?): View {
        return (convertView as? TextView ?: inflater.inflate(dropdownRes, parent, false) as TextView).apply {
            text = getItem(position).shortName
        }
    }

    companion object {
        private const val dropdownRes = android.support.v7.appcompat.R.layout.support_simple_spinner_dropdown_item
    }
