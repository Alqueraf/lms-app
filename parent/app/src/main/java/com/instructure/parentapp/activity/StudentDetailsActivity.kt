...
class StudentDetailsActivity : BaseParentActivity(), StudentThresholdDialog.StudentThresholdChanged {
    ...
    private fun getThresholdIdByAlertType(alertType: Alert.ALERT_TYPE): String {
    ...
    private fun removeThresholdByAlertType(alertType: Alert.ALERT_TYPE) {
    ...
    private fun deleteThreshold(alertType: Alert.ALERT_TYPE) {
        ...
        if (thresholdId != "-1") {
            ...
            thresholdJob = tryWeave {
                ...
                when (alertType) {
                    Alert.ALERT_TYPE.COURSE_GRADE_LOW -> hasGradeBelowThreshold = false
                    Alert.ALERT_TYPE.COURSE_GRADE_HIGH -> hasGradeAboveThreshold = false
                    Alert.ALERT_TYPE.ASSIGNMENT_MISSING -> hasAssignmentMissingThreshold = false
                    Alert.ALERT_TYPE.COURSE_ANNOUNCEMENT -> hasTeacherAnnouncementsThreshold = false
                    Alert.ALERT_TYPE.INSTITUTION_ANNOUNCEMENT -> hasInstitutionAnnouncementsThreshold = false
                    Alert.ALERT_TYPE.ASSIGNMENT_GRADE_HIGH -> hasAssignmentGradeAboveThreshold = false
                    Alert.ALERT_TYPE.ASSIGNMENT_GRADE_LOW -> hasAssignmentGradeBelowThreshold = false
                }
    ...
    private fun updateAlertThreshold(newThreshold: String, alertType: Alert.ALERT_TYPE) {
        AnalyticUtils.trackButtonPressed(AnalyticUtils.MODIFY_THRESHOLD)
    ...
    private fun updateAlertThreshold(alertType: Alert.ALERT_TYPE) {
        AnalyticUtils.trackButtonPressed(AnalyticUtils.MODIFY_THRESHOLD)
        ...
        if (thresholdId != "-1") {
            ...
            thresholdJob = tryWeave {
                awaitApi<ObserverAlertThreshold> {  AlertThresholdManager.updateObserverAlertThreshold(
                        thresholdId,
                        Alert.alertTypeToAPIString(alertType), null, it) }
    ...
    private fun createThreshold(alertType: Alert.ALERT_TYPE) {
        AnalyticUtils.trackButtonPressed(AnalyticUtils.MODIFY_THRESHOLD)
    ...
    private fun createThreshold(threshold: String, alertType: Alert.ALERT_TYPE) {
        AnalyticUtils.trackButtonPressed(AnalyticUtils.MODIFY_THRESHOLD)
     ...
     * @param alertType
     */
    fun enableSwitches(alertType: Alert.ALERT_TYPE) {
        ...
        for (threshold in alertThresholds) {
            if (!TextUtils.isEmpty(threshold.alertType) && Alert.getAlertTypeFromString(threshold.alertType) != null) {
                when (Alert.getAlertTypeFromString(threshold.alertType)) {
                    Alert.ALERT_TYPE.COURSE_GRADE_HIGH -> if (!TextUtils.isEmpty(threshold.threshold)) {
                        gradeAboveValue.text = threshold.threshold + "%"

                        hasGradeAboveThreshold = true
                    }
                    Alert.ALERT_TYPE.COURSE_GRADE_LOW -> if (!TextUtils.isEmpty(threshold.threshold)) {
                        gradeBelowValue.text = threshold.threshold + "%"

                        hasGradeBelowThreshold = true
                    }
                    Alert.ALERT_TYPE.COURSE_ANNOUNCEMENT -> {
                    ...
                    Alert.ALERT_TYPE.ASSIGNMENT_GRADE_HIGH -> if (!TextUtils.isEmpty(threshold.threshold)) {
                        assignmentGradeAboveValue.text = threshold.threshold + "%"

                        hasAssignmentGradeAboveThreshold = true
                    }
                    Alert.ALERT_TYPE.ASSIGNMENT_GRADE_LOW -> if (!TextUtils.isEmpty(threshold.threshold)) {
                        assignmentGradeBelowValue.text = threshold.threshold + "%"

                        hasAssignmentGradeBelowThreshold = true
                    }
                    Alert.ALERT_TYPE.INSTITUTION_ANNOUNCEMENT -> {
