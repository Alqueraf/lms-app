...
class ProfileSettingsFragment : ParentFragment(), LoaderManager.LoaderCallbacks<AvatarWrapper> {
    ...
    private fun setupViews() {
        ...
        editUsername.onClickWithRequireNetwork {
            EditTextDialog.show(fragmentManager, getString(R.string.editUserName), ApiPrefs.user?.name.orEmpty()) {
                updateUserName(it)
            }
        }
        createPandaAvatar.onClickWithRequireNetwork {
            startActivityForResult(Intent(activity, PandaAvatarActivity::class.java), Const.PANDA_AVATAR_RESULT_CODE)
        }
    }
    ...
    private fun updateUserName(newName: String) {
        ...
        mUpdateNameCall = tryWeave {
            ...
            ApiPrefs.user = ApiPrefs.user?.apply { shortName = user.shortName }
    ...
    private fun takeNewPhotoBecausePermissionsAlreadyGranted() {
        ...
        mCapturedImageUri = context.contentResolver.insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, values)
    ...
    override fun onCreateLoader(id: Int, args: Bundle): android.support.v4.content.Loader<AvatarWrapper> {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == RequestCodes.CROP_IMAGE && resultCode == Activity.RESULT_OK) {
            ...
            val cropIntent = AvatarCropActivity.createIntent(context, config)
