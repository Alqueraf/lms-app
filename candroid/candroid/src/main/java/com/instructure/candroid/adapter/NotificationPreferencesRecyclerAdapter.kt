...
class NotificationPreferencesRecyclerAdapter(context: Context) : ExpandableRecyclerAdapter<NotificationCategoryHeader, NotificationCategory, RecyclerView.ViewHolder>(context, NotificationCategoryHeader::class.java, NotificationCategory::class.java ) {
    ...
    override fun createViewHolder(v: View, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType == Types.TYPE_HEADER) {
            NotificationPreferencesHeaderViewHolder(v)
        } else {
            NotificationPreferencesViewHolder(v)
        }
    }
    ...
    override fun itemLayoutResId(viewType: Int): Int {
        return if (viewType == Types.TYPE_HEADER) {
            NotificationPreferencesHeaderViewHolder.holderResId()
        } else {
            NotificationPreferencesViewHolder.holderResId()
        }
    }
    ...
    fun fetchNotificationPreferences(channel: CommunicationChannel) {
        ...
        mApiCall = tryWeave {
            val response = awaitApi<NotificationPreferenceResponse> {
                NotificationPreferencesManager.getNotificationPreferences(channel.user_id, channel.id, true, it)
            }
