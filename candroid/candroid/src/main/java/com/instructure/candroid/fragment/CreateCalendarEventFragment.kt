...
class CreateCalendarEventFragment : ParentFragment(), TimePickerFragment.TimePickerCancelListener {
    ...
    private fun initViews() {
        ...
        eventStartTimeText.text = DateHelper.getDayHourDateString(context, startCalendar.time)
        eventEndTimeText.text = DateHelper.getDayHourDateString(context, endCalendar.time)
    }

    private fun setUpListeners() {
        eventDateText.onClick {
            DatePickerDialogFragment.getInstance(activity.supportFragmentManager, null) { year, month, dayOfMonth ->
                startCalendar.set(year, month, dayOfMonth)
                endCalendar.set(year, month, dayOfMonth)
                eventDateText.text = getFullDateString(startCalendar.time)
            }.show(activity.supportFragmentManager, TAG_DATE_PICKER)
        }

        eventStartTimeText.onClick {
            TimePickerDialogFragment.getInstance(activity.supportFragmentManager, startCalendar.time) { hour, minute ->
                startCalendar.apply {
            ...
            }.show(activity.supportFragmentManager, TAG_TIME_PICKER_START)
        }

        eventEndTimeText.onClick {
            TimePickerDialogFragment.getInstance(activity.supportFragmentManager, endCalendar.time) { hour, minute ->
                endCalendar.apply {
                    set(Calendar.HOUR_OF_DAY, hour)
                    set(Calendar.MINUTE, minute)
                }
            ...
            }.show(activity.supportFragmentManager, TAG_TIME_PICKER_END)
        }
    }
        ...
        saveJob = tryWeave {
            val item: ScheduleItem = awaitApi {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    description = Html.fromHtml(eventNoteText.text.toString(), Html.FROM_HTML_MODE_LEGACY).toString()
                } else {
                    description = Html.fromHtml(eventNoteText.text.toString()).toString()
                }
                CalendarEventManager.createCalendarEvent(
                    ApiPrefs.user!!.contextId,
                    titleEditText.text.toString(),
                    description,
                    APIHelper.dateToString(startCalendar.time),
                    APIHelper.dateToString(endCalendar.time),
    ...
    private fun getFullDateString(date: Date?): String {
        if (date == null) return ""
        return "${DateHelper.getFullDayFormat().format(date)} ${DateHelper.getFormattedDate(context, date)}"
    }
