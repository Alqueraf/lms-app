...
class CourseModuleProgressionFragment : ParentFragment(), Bookmarkable {
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        ...
        prev_item.background = ColorKeeper.getColoredDrawable(activity, R.drawable.left_arrow, canvasContext)
        next_item.background = ColorKeeper.getColoredDrawable(activity, R.drawable.right_arrow, canvasContext)
    }
    ...
    private fun setButtonListeners() {
        ...
        markDoneButton.setOnClickListener {
            if (getModelObject() != null && getModelObject()!!.completionRequirement != null) {
                if (getModelObject()!!.completionRequirement.isCompleted) {
                    ModuleManager.markAsNotDone(canvasContext, getModelObject()!!.moduleId, getModelObject()!!.id,
                            object : StatusCallback<ResponseBody>() {
                                override fun onResponse(response: Response<ResponseBody>, linkHeaders: LinkHeaders, type: ApiType) {
                                    ...
                                    getModelObject()!!.completionRequirement.isCompleted = false
                    ...
                    ModuleManager.markAsDone(canvasContext, getModelObject()!!.moduleId, getModelObject()!!.id,
                            object : StatusCallback<ResponseBody>() {
                                override fun onResponse(response: Response<ResponseBody>, linkHeaders: LinkHeaders, type: ApiType) {
                                    ...
                                    getModelObject()!!.completionRequirement.isCompleted = true
                                    notifyOfItemChanged(getModelObject())
                                }
                            })
                }
    ...
    private fun updateBottomNavBarButtons() {
        ...
        if (completionRequirement != null && ModuleItem.MUST_VIEW == completionRequirement.type) {
    ...
    private fun updateModuleMarkDoneView(item: ModuleItem?) {
        ...
        if (item == null) {
            ...
            if (completionRequirement != null && ModuleItem.MUST_MARK_DONE == completionRequirement.type) {
                ...
                markDoneCheckbox.isChecked = completionRequirement.isCompleted
            } else {
                markDoneWrapper.setGone()
            }
        }
    ...
    private fun getModuleItemData(moduleId: Long) {
        moduleItemsJob = tryWeave {
            ...
            for (i in moduleItems.indices) {
                ...
                if (shouldAddModuleItem(this@CourseModuleProgressionFragment.context, moduleItems[i]) && !items[index].contains(moduleItems[i])) {
    ...
    private fun addLockedIconIfNeeded(objects: ArrayList<ModuleObject>, moduleItems: ArrayList<ArrayList<ModuleItem>>, groupPosition: Int, childPosition: Int): Boolean {
        ...
        if (objects[groupPosition].isSequential_progress
            ...
            for (i in 0 until moduleItems[groupPosition].size) {
                if (moduleItems[groupPosition][i].completionRequirement != null && !moduleItems[groupPosition][i].completionRequirement.isCompleted) {
                    // i is the first non completed item, so we don't include this item to show in the view pager.
                    index = i
                    break
                }
            }
            // Index is now the index of the first non completed item. We show the first non completed item, but not the next_item one, which is why this is a
    ...
    inner class CourseModuleProgressionAdapter(fm: FragmentManager) : FragmentStatePagerAdapter(fm) {
        ...
        override fun getItem(position: Int): Fragment {
            ...
            val moduleItem = getCurrentModuleItem(position)
    ...
    companion object {
        ...
        fun shouldAddModuleItem(context: Context, moduleItem: ModuleItem): Boolean = when {
            moduleItem.type == "unlock_requirements" -> false
