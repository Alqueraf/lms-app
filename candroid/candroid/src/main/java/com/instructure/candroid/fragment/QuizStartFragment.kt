...
class QuizStartFragment : ParentFragment(), Bookmarkable {
    ...
    override fun title(): String =if(quiz.title.isNullOrBlank()) getString(R.string.quizzes) else quiz.title
    ...
    private fun setupViews() {
        ...
        quiz_results.setOnClickListener {
            ...
            RouteMatcher.route(activity, InternalWebviewFragment.makeRoute(course, quiz.url, false, false, true))
        }
    }
    ...
    fun populateQuizInfo() {
        ...
        quiz_points_details.text = quiz.pointsPossible
        ...
        if (quiz.dueAt != null) {
            quiz_due_details.text = DateHelper.getDateTimeString(activity, quiz.dueAt)
        } else {
        ...
        if (quiz.unlockAt != null) {
            ...
            quiz_unlocked_details.text = DateHelper.getDateTimeString(activity, quiz.unlockAt)
        } else {
    ...
    private fun setupCallbacks() {
        ...
        quizSubmissionResponseCanvasCallback = object : StatusCallback<QuizSubmissionResponse>() {
            override fun onResponse(response: Response<QuizSubmissionResponse>, linkHeaders: LinkHeaders, type: ApiType) {
                ...
                if (user != null) {
                    for (submission in quizSubmissionResponse!!.quizSubmissions) {
                ...
                quizSubmissionResponse!!.quizSubmissions = submissions
                if (quizSubmissionResponse.quizSubmissions == null || quizSubmissionResponse.quizSubmissions.size == 0) {
                    ...
                    val teacherUnlockedQuizAttempts = quizSubmission!!.isManuallyUnlocked // Teacher can manually unlock a quiz for an individual student
                    ...
                    if (quiz.hideResults == Quiz.HIDE_RESULTS_TYPE.ALWAYS && !canTakeQuizAgain) {
                        // Don't let the user see the questions if they've exceeded their attempts
                        next.setGone()
                    } else if (quiz.hideResults == Quiz.HIDE_RESULTS_TYPE.AFTER_LAST_ATTEMPT && !canTakeQuizAgain) {
                    ...
                    if (quizSubmission!!.finishedAt != null) {
                        ...
                        quiz_turned_in_details.text = DateHelper.getDateTimeString(activity, quizSubmission!!.finishedAt)
                    ...
                    if (quizSubmission!!.workflowState == QuizSubmission.WORKFLOW_STATE.UNTAKEN && quizSubmission!!.endAt != null && quizSubmissionTime != null && quizSubmissionTime!!.timeLeft > 0) {
                        next.isEnabled = false
                        ...
                        QuizManager.submitQuiz(course, quizSubmission, true, object : StatusCallback<QuizSubmissionResponse>() {
                            override fun onResponse(response: Response<QuizSubmissionResponse>, linkHeaders: LinkHeaders, type: ApiType) {
                                ...
                                if (userVal != null) {
                                    for (submission in quizResponse!!.quizSubmissions) {
                                ...
                                quizResponse!!.quizSubmissions = submissionsVal

                                if (quizResponse.quizSubmissions != null && quizResponse.quizSubmissions.size > 0) {
                    ...
                    if (quiz.isOneTimeResults && quizSubmission!!.hasSeenResults() && !canTakeQuizAgain) {
                        next.setGone()
                    }
    ...
    private fun showQuiz() {
        ...
        if (navigation != null) {
            ...
            RouteMatcher.route(activity, QuizQuestionsFragment.makeRoute(canvasContext, quiz, quizSubmission, shouldLetAnswer))
        }
    }
