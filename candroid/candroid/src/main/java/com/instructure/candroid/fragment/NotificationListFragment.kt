...
class NotificationListFragment : ParentFragment(), Bookmarkable {
    ...
    fun isCourseOrGroup(): Boolean { // used to help determine if the bottom bar should be highlighted
        return canvasContext.isCourseOrGroup
    }

    override fun title(): String {
        return getString(if (canvasContext.isCourse || canvasContext.isGroup) R.string.homePageIdForNotifications else R.string.notifications)
    }

    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return layoutInflater.inflate(R.layout.fragment_list_notification, container, false)
    }

    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        recyclerAdapter =
                NotificationListRecyclerAdapter(context, canvasContext, onNotificationCountInvalidated, adapterToFragmentCallback)
    ...
    companion object {
        ...
        fun addFragmentForStreamItem(streamItem: StreamItem, context: Context, fromWidget: Boolean) {
            ...
            if (fromWidget) {
                RouteMatcher.routeUrl(context, streamItem.url ?: streamItem.htmlUrl)
                return
            }
            
            if (streamItem.type == CONVERSATION) {
            ...
            val route: Route? = when (streamItem.type) {
                SUBMISSION -> {
                    ...
                    if (streamItem.assignment == null) {
                        ...
                        streamItem.assignment?.submission = Submission().apply { grade = streamItem.grade }
                ...
                else -> UnsupportedFeatureFragment.makeRoute(canvasContext, streamItem.type.toString(), streamItem.url)
            }
        ...
        fun makeRoute(canvasContext: CanvasContext): Route {
            return Route(NotificationListFragment::class.java, canvasContext, Bundle())
        }
