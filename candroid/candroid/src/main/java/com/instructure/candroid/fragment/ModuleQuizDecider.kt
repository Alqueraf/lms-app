...
class ModuleQuizDecider : ParentFragment() {
    ...
    private fun obtainQuiz() {
        tryWeave {
            ...
            if (quiz.dueAt != null) {
                quizDueDetails.text = DateHelper.getDateTimeString(activity, quiz.dueAt)
            } else {
                quizDue.setGone()
                quizDueDetails.text = getString(R.string.toDoNoDueDate)
            }
            quizDetails.formatHTML(quiz.description, "")
            quizDetails.setBackgroundColor(ContextCompat.getColor(activity, R.color.transparent))
