...
class PeopleDetailsFragment : ParentFragment(), Bookmarkable {
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        ...
        compose.setIconDrawable(ColorKeeper.getColoredDrawable(context, R.drawable.vd_send, Color.WHITE))
    ...
    private fun fetchUser() {
        userCall = tryWeave {
            user = awaitApi { UserManager.getUserForContextId(canvasContext, userId, it, true) }
            setupUserViews()
        } catch {
    ...
    override fun applyTheme() {
        ViewStyler.setStatusBarDark(activity, ColorKeeper.getOrGenerateColor(canvasContext))
    }

    private fun setupUserViews() {
        user?.let {
            ProfileUtils.loadAvatarForUser(avatar, it)
            userName.text = it.name
            userRole.text = it.enrollments.joinToString { it.displayType }
            userBackground.setBackgroundColor(ColorKeeper.getOrGenerateColor(canvasContext))
            bioText.setVisible(it.bio.isValid() && it.bio != null).text = it.bio
        }
    }
