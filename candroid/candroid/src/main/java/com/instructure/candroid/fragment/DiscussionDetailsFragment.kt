...
class DiscussionDetailsFragment : ParentFragment(), Bookmarkable {
    ...
    override fun title(): String = if(discussionTopicHeaderId == 0L) discussionTopicHeader.title ?: getString(R.string.discussion) else getString(R.string.discussion)
    ...
    private fun showReplyView(discussionEntryId: Long) {
        if (APIHelper.hasNetworkConnection()) {
            scrollPosition = discussionsScrollView.scrollY
            val route = DiscussionsReplyFragment.makeRoute(canvasContext, discussionTopicHeader.id, discussionEntryId, discussionTopicHeader.permissions.isAttach)
    ...
    private inner class JSDiscussionInterface {
        ...
        @JavascriptInterface fun onReplyPressed(id: String) = showReplyView(id.toLong())
        @JavascriptInterface fun onEditPressed(id: String) = showUpdateReplyView(id.toLong())
        @JavascriptInterface fun onDeletePressed(id: String) = askToDeleteDiscussionEntry(id.toLong())
        @JavascriptInterface fun onLikePressed(id: String) = likeDiscussionPressed(id.toLong())
        ...
        @JavascriptInterface fun getInViewPort(): String = discussionTopic?.unreadEntries?.joinToString() ?: ""
        ...
        // A helper to log out messages from the JS code
        @JavascriptInterface fun logMessage(message: String) = Logger.d(message)
    ...
    private fun populateDiscussionData(forceRefresh: Boolean = false) {
        discussionsLoadingJob = tryWeave {
            ...
            if(forceRefresh || discussionTopic == null) {
                ...
                discussionTopic = if (discussionTopicHeader.groupTopicChildren.isNotEmpty()) {
                    ...
                    var groupDiscussionIds = GroupTopicChild(0, 0)
                    userGroups.forEach { userGroup ->
                        discussionTopicHeader.groupTopicChildren.forEach {
                            if (userGroup == it.groupId)
                                groupDiscussionIds = it
                        }
                    }

                    awaitApi<DiscussionTopic> { DiscussionManager.getFullDiscussionTopic(CanvasContext.emptyGroupContext(groupDiscussionIds.groupId), groupDiscussionIds.id, true, it)}
                } else {
                    // Regular discussion, fetch normally
                    awaitApi<DiscussionTopic> { DiscussionManager.getFullDiscussionTopic(canvasContext, discussionTopicHeader.id, true, it) }
                }
