...
class AddSubmissionFragment : ParentFragment(), Bookmarkable {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        if (!isOnlineTextAllowed && !isUrlEntryAllowed) {
            if (isMediaUploadAllowed && !isFileUploadAllowed) {
                val intent = NotoriousMediaUploadPicker.createIntentForAssignmentSubmission(context, assignment)
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        ...
        ViewStyler.themeEditText(context, textEntry, ThemePrefs.brandColor)
        ViewStyler.themeEditText(context, onlineURL, ThemePrefs.brandColor)
        ViewStyler.themeEditText(context, arcEntry, ThemePrefs.brandColor)
    }
    ...
    override fun onStart() {
        ...
        LocalBroadcastManager.getInstance(context).registerReceiver(mArcSubmissionReceiver, IntentFilter(Const.ARC_SUBMISSION))
    }
    ...
    override fun onStop() {
        ...
        LocalBroadcastManager.getInstance(context).unregisterReceiver(mArcSubmissionReceiver)
    }
    //endregion
    ...
    private fun setupViews() {
        ...
        if (isOnlineTextAllowed && !isUrlEntryAllowed && !isFileUploadAllowed) {
            VisibilityAnimator.animateVisible(AnimationUtils.loadAnimation(activity,
            ...
            override fun onPageFinished(view: WebView, finishedURL: String) {
                ...
                delayLoadJob = weave {
                    ...
                    if (Patterns.WEB_URL.matcher(url).matches()) {
                        ...
                        onlineURL.setSelection(onlineURL.text.length)
                    }
                }
            }
        }
    ...
    private fun setupListeners() {
        ...
        onlineURLHeader.setOnClickListener {
            if (urlEntryContainer.visibility == View.VISIBLE) {
                VisibilityAnimator.animateGone(AnimationUtils.loadAnimation(activity,
        ...
        arcSubmission.setOnClickListener {
            ...
            RouteMatcher.route(activity, ArcWebviewFragment.makeRoute(canvasContext, url, arcLTITool!!.name, true))
        }
    ...
    private fun tryToSubmitText() {
        ...
        val textToSubmit = textEntry.text.toString().replace("\\n".toRegex(), "<br/>")
        if (textToSubmit.trim { it <= ' ' }.isEmpty()) {
            // It's an empty submission
            toast(R.string.blankSubmission, Toast.LENGTH_LONG)
        } else {
            // Log to GA
            Analytics.trackButtonPressed(activity, "Submit Text Assignment", null)
    ...
    private fun tryToSubmitURL() {
        ...
        if (urlToSubmit.trim { it <= ' ' }.isEmpty()) {
            ...
            // Log to GA
            Analytics.trackButtonPressed(activity, "Submit URL Assignment", null)
    ...
    private fun tryToSubmitArc() {
        ...
        if (urlToSubmit.trim { it <= ' ' }.isEmpty()) {
            ...
            // Log to GA
            Analytics.trackButtonPressed(activity, "Submit Arc Assignment", null)
