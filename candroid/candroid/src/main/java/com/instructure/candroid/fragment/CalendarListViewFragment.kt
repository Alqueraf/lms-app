...
import com.instructure.candroid.events.CalendarEventCreated
...
class CalendarListViewFragment : ParentFragment() {
    ...
    private var calendarFragment: CanvasCalendarFragment? = null
    ...
    private var emptyPandaView: LinearLayout? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (recyclerAdapter == null) {
            setUpCallbacks()
            recyclerAdapter = CalendarListRecyclerAdapter(context, adapterToFragmentCallback, adapterToCalendarCallback)
        }
    ...
    private fun onRowClick(scheduleItem: ScheduleItem) {
        ...
        if (scheduleItem.itemType == ScheduleItem.Type.TYPE_ASSIGNMENT) {
            RouteMatcher.routeUrl(activity, scheduleItem.htmlUrl)
    ...
    private fun populateView(inflater: LayoutInflater, container: ViewGroup?): View {
        ...
        emptyPandaView = rootView.findViewById(R.id.calendarEmptyView)
        emptyTextView = emptyPandaView?.findViewById(R.id.noItems)
        configureRecyclerView(rootView, context, recyclerAdapter!!, R.id.calendarSwipeRefreshLayout, R.id.calendarEmptyView, R.id.calendarRecyclerView)
    ...
    private fun setUpCallbacks() {
        adapterToCalendarCallback = object : CalendarListRecyclerAdapter.AdapterToCalendarCallback {
            override fun showChooserDialog(firstShow: Boolean) {
                showCalendarChooserDialog(firstShow)
            }

            override fun hidePandaLoading() {
                hidePanda()
            }

            override fun showPandaLoading() {
                showPanda()
            }

            override fun getCurrentCalendarView(): Int {
                return CalendarView.toInteger(this@CalendarListViewFragment.currentCalendarView)
            }

            override fun getExtraCalendarData(): HashMap<String, Any>? {
                return calendarFragment?.extraData
            }

            override fun refreshCalendarFragment() {
                calendarFragment?.refreshView()
            }

            override fun setSelectedDates(d1: Date, d2: Date) {
                calendarFragment?.setSelectedDates(d1, d2)
            }
        }

        adapterToFragmentCallback = object : AdapterToFragmentCallback<ScheduleItem> {
            override fun onRowClicked(item: ScheduleItem, position: Int, isOpenDetail: Boolean) {
                onRowClick(item)
            }
    ...
    private fun eventCreation() {
        ...
        Analytics.trackAppFlow(activity, CreateCalendarEventFragment::class.java)
        val time: Long? = recyclerAdapter?.selectedDay?.getMilliseconds(TimeZone.getDefault())
        RouteMatcher.route(context, CreateCalendarEventFragment.makeRoute(time ?: 0L))
    }

    private fun showCalendarChooserDialog(firstShow: Boolean) {
        recyclerAdapter?.let {
            CalendarChooserDialogStyled.show(activity, CalendarListRecyclerAdapter.getFilterPrefs(), it.contextNames, it.contextCourseCodes, firstShow, dialogCallback)
        }
    }
    ...
    private fun setupActionbarSpinnerForMonth() {
        ...
        monthContainer = LayoutInflater.from(context).inflate(R.layout.actionbar_calendar_layout, null)
        toolbarContainer?.removeAllViews()
        toolbarContainer?.addView(monthContainer)

        dropDownIndicator = monthContainer?.findViewById(R.id.indicator)
        dropDownIndicator?.setImageDrawable(ColorKeeper.getColoredDrawable(context, R.drawable.vd_expand, ThemePrefs.primaryTextColor))
        ...
        if (calendarFragment?.currentMonth != -1) {
            ...
            // 1 indexed months.
            monthText?.text = DateFormatSymbols().months[calendarFragment?.currentMonth?.minus(1) ?: 0] + " " + calendarFragment?.currentYear
        }
    ...
    private fun expandOrCollapseCalendar(calendarView: View, isExpand: Boolean) {
        ...
        if (isLandscape) {
            calendarView.animate().translationX(translation.toFloat()).setListener(onFinished)

        } else {
            calendarView.animate().translationY(translation.toFloat()).setListener(onFinished)
        }
    }
    ...
    private fun showPanda() {
        ...
        ListViewHelpers.changeEmptyViewToLoading(emptyPandaView)
    }
    ...
    private fun hidePanda() {
        ...
        ListViewHelpers.changeEmptyViewToNoItems(emptyPandaView)
