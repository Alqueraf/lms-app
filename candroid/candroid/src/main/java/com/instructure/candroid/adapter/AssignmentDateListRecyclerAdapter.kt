...
open class AssignmentDateListRecyclerAdapter(
    ...
    init {
        overdue = AssignmentGroup(context.getString(R.string.overdueAssignments), HEADER_POSITION_OVERDUE)
        upcoming = AssignmentGroup(context.getString(R.string.upcomingAssignments), HEADER_POSITION_UPCOMING)
        undated = AssignmentGroup(context.getString(R.string.undatedAssignments), HEADER_POSITION_UNDATED)
        past = AssignmentGroup(context.getString(R.string.pastAssignments), HEADER_POSITION_PAST)
    ...
    private fun addAndSortAssignments(assignmentGroups: List<AssignmentGroup>) {
        val today = Date()
        for (assignmentGroup in assignmentGroups) {
            ...
            // endtodo
            for (assignment in assignmentGroup.assignments) {
                val dueDate = assignment.dueAt
                ...
                if (dueDate == null) {
                    ...
                    when {
                        today.before(dueDate) -> addOrUpdateItem(upcoming, assignment)
    ...
    override fun createItemCallback() = object : GroupSortedList.ItemComparatorCallback<AssignmentGroup, Assignment> {
        ...
        override fun compare(group: AssignmentGroup, o1: Assignment, o2: Assignment): Int {
            ...
            return when (position) {
                HEADER_POSITION_UNDATED -> o1.name.toLowerCase().compareTo(o2.name.toLowerCase())
                HEADER_POSITION_PAST -> o2.dueAt?.compareTo(o1.dueAt) ?: 0 // Sort newest date first (o1 and o2 switched places)
                else -> o1.dueAt!!.compareTo(o2.dueAt) // Sort oldest date first
            }
        }
    }
